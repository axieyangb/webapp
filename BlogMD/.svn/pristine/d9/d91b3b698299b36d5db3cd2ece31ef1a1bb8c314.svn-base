...
var Skype = new function () {
    ...
    this.assetPrefix = "/Content/Imgs/skypeChatIcon/";
		...
		function l(L) {
		    ...
		    z.setAttribute("style", ("height:13px"));
			if (this.name === this.dropdownLinks.name) {
			...
			N.appendChild(z);
			N.innerHTML+="<span style='font-family: \"Roboto\",\"Noto Sans SC\";font-size:14px;'>yang.yang.xie</span>";
		...
		function j(u, r) {
			...
			this.chatLinks.linkImage = this.assetPrefix + "chatbutton_14px.png";
		...
		function h(r) {
			if(!r) {
				...
				for(var i = 0; i < is.length; i++) {
					if(is[i].id && is[i].id.match("SkypeButton")) {
						r = is[i];
					}
				}
			}
